defimpl ElixirBrod.Avro.ModuleWriter.Metadata, for: ElixirBrod.Avro.SchemaParser.Record do
  ...
  defp elixir_type_from_avro_field(_type, %Field{logical_type: logical}) when logical in @logical_types,
  ...
  defp elixir_type_from_avro_field(%Field{name: name}, _field), do: String.replace(name, ~r/\s/, "_")
    
  defp validate(%Field{name: name, type: type} = definition) when type in @simple_types,
       do: "ElixirBrod.Avro.Validation.Simple.validate(#{inspect String.to_existing_atom(name)}, #{inspect definition})"

  defp validate(%Field{name: name, type: type} = definition) when type in @complex_types or is_list(type),
       do: "ElixirBrod.Avro.Validation.Complex.validate(#{inspect String.to_existing_atom(name)}, #{inspect definition})"
end
